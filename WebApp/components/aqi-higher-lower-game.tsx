...
interface CityAQI { name: string; aqi: number; }
...
export function AQIHigherLowerGame() {
  ...
  const [status, setStatus] = useState<"playing" | "reveal" | "gameover" | "loading" | "error">("loading");
  ...
  const fetchRandomCities = useCallback(async (initial = false) => {
    try {
      ...
      const newCities: CityAQI[] = points
        .filter(p => typeof p.location === "string" && p.location.trim().length > 0 && typeof p.value === "number")
        .map(p => ({ name: p.location as string, aqi: Math.round(p.value) }));
  ...
  const remainingCities = useMemo(() => {
    if (!current) return [];
    return cities.filter(c => c.name !== current.name && c.name !== nextCity?.name);
  ...
  const handleGuess = async (guess: "higher" | "lower") => {
    ...
    if (correct) {
      setScore(s => s + 1);
      setStatus("reveal");
      setMessage(`Correct! ${nextCity.name} has an AQI of ${nextCity.aqi}.`);
      setTimeout(async () => {
        await ensurePool();
      ...
      <Card className="bg-gradient-to-br from-background/80 to-background/40 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <CardHeader className="pb-2">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button size="sm" variant="ghost" onClick={restart} disabled={status === 'loading'}>
              Refresh Pool
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {status === "loading" && (
            <div className="flex flex-col items-center py-10 gap-3 text-sm text-muted-foreground">
              <Loader2 className="h-5 w-5 animate-spin" /> Loading city AQI data...
            </div>
          )}
          {status === "error" && (
          ...
          {current && nextCity && status !== "loading" && status !== "error" && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-lg border bg-card/40">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground mb-1">Current City</p>
                  <p className="font-semibold">{current.name}</p>
                  <p className="text-3xl font-bold mt-2">{current.aqi}</p>
                  <p className="text-xs text-muted-foreground mt-1">AQI</p>
                </div>
                <div className="p-4 rounded-lg border bg-card/40 relative">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground mb-1">Next City</p>
                  <p className="font-semibold">{nextCity.name}</p>
                  <p className="text-3xl font-bold mt-2">
                    {status === "reveal" || status === "gameover" ? nextCity.aqi : "?"}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">AQI</p>
                  {status === "playing" && (
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <div>Remaining: {remainingCities.length}</div>
                <div className="flex gap-4 items-center">
                  {fetchingMore && <Loader2 className="h-3 w-3 animate-spin" />}
                  <span>Score: <span className="font-medium text-foreground">{score}</span></span>
                  <span className="hidden sm:inline">|</span>
                  <span className="flex items-center gap-1"><Trophy className="h-3.5 w-3.5" /> High: {highScore}</span>
                </div>
              </div>
              ...
              {status === "playing" && (
                <div className="flex gap-3">
                  <Button className="flex-1" onClick={() => handleGuess("higher")}>Higher</Button>
                  <Button className="flex-1" variant="secondary" onClick={() => handleGuess("lower")}>Lower</Button>
                </div>
              )}

              {status === "reveal" && (
                <div className="text-center text-sm font-medium text-emerald-500 animate-pulse">{message}</div>
              )}
              ...
              {status === "gameover" && (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <Button size="sm" onClick={restart}>
                    <RefreshCw className="h-4 w-4 mr-1" /> Play Again
                  </Button>
                </div>
              )}
            </div>
